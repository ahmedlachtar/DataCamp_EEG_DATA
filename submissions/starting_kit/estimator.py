...
def feature_extractor(X):
    ...
    for k, (_, x) in enumerate(X_df.iteritems()):
        ...
        for i in range(n_channels):
            ...
            # wv_feat.append(np.quantile(cat, 0.25))
            # wv_feat.append(np.quantile(cat, 0.5))
            # wv_feat.append(np.quantile(cat, 0.75))
            #wv_feat.append(np.std(cat))
            wv_feat.append(np.mean(cdt))
            wv_feat.append(np.max(cdt))
            wv_feat.append(np.min(cdt))
            # wv_feat.append(np.quantile(cdt, 0.25))
            # wv_feat.append(np.quantile(cdt, 0.5))
            # wv_feat.append(np.quantile(cdt, 0.75))
